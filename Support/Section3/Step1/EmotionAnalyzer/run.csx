...
public static async void Run (CloudBlockBlob myBlob, string name, CloudTable outputTable, TraceWriter log) {
    ...
    string endpoint = "https://[location].api.cognitive.microsoft.com/face/v1.0/detect";
        ...
        using (var content = new System.Net.Http.ByteArrayContent (buf)) {
            ...
            if (response.StatusCode == System.Net.HttpStatusCode.OK) {
                ...
                foreach (dynamic re in res) {
                    ...
                    dynamic angerToken = re.SelectToken ("faceAttributes.emotion.anger");
                    double anger = angerToken.Value;
                    dynamic contemptToken = re.SelectToken ("faceAttributes.emotion.contempt");
                    double contempt = contemptToken.Value;
                    dynamic disgustToken = re.SelectToken ("faceAttributes.emotion.disgust");
                    double disgust = disgustToken.Value;
                    dynamic fearToken = re.SelectToken ("faceAttributes.emotion.fear");
                    double fear = fearToken.Value;
                    dynamic happinessToken = re.SelectToken ("faceAttributes.emotion.happiness");
                    double happiness = happinessToken.Value;
                    dynamic neutralToken = re.SelectToken ("faceAttributes.emotion.neutral");
                    double neutral = neutralToken.Value;
                    dynamic sadnessToken = re.SelectToken ("faceAttributes.emotion.sadness");
                    double sadness = sadnessToken.Value;
                    dynamic surpriseToken = re.SelectToken ("faceAttributes.emotion.surprise");
